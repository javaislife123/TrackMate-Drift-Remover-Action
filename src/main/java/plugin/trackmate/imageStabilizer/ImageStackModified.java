...
@see ImagePlus
*/
public class ImageStackModified extends ImageStack{
	...
	/** Default constructor. */
	public ImageStackModified() { }

	/** Creates a new, empty image stack. */
	public ImageStackModified(int width, int height) {
	...
	public ImageStackModified(int width, int height, int size) {
		this.width = width;
		this.height = height;
	...
	public ImageStackModified(int width, int height, ColorModel cm) {
		this.width = width;
		this.height = height;
		this.cm = cm;
	...
	*/
	 public static ImageStack create(int width, int height, int depth, int bitdepth) {
		ImageStack stack =  (ImageStack)IJ.createImage("", width, height, depth, bitdepth).getStack();
		
		return stack;
	 }
	 
	/** Creates an ImageStack from an ImagePlus array. */
	 public static ImageStackModified create(ImagePlus[] images) {
		int w = 0;
		int h = 0;
		...
		ImageStackModified stack =  new ImageStackModified(w, h);
		stack.init(w, h);
		for (int i=0; i<images.length; i++) {
	...
	/** Duplicates this stack. */
	 public ImageStackModified duplicate() {
	 	return crop(0, 0, 0, width, height, size());
	 }
	 
	/** Creates a new stack by cropping this one. */
	 public ImageStackModified crop(int x, int y, int z, int width, int height, int depth) {
	 	if (x<0||y<0||z<0||x+width>this.width||y+height>this.height||z+depth>size())
	 		throw new IllegalArgumentException("Argument out of range");
		ImageStackModified stack2 = new ImageStackModified(width, height, getColorModel());
	...
	/** Creates a float version of this stack. */
	 public ImageStackModified convertToFloat() {
		ImageStackModified stack2 = new ImageStackModified(width, height, getColorModel());
